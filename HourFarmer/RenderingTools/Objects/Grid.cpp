...
void RT::Grid::Draw(CanvasWrapper canvas, Frustum &frustum, bool useThickMidline) const
{
	...
	Line(corners[0], corners[1], 3).DrawWithinFrustum(canvas, frustum);
	Line(corners[1], corners[2], 3).DrawWithinFrustum(canvas, frustum);
	Line(corners[2], corners[3], 3).DrawWithinFrustum(canvas, frustum);
	Line(corners[3], corners[0],3 ).DrawWithinFrustum(canvas, frustum);
